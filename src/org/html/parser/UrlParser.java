...
//				log.info( currLine );
				...
				{
					setEof();
					throw new UrlParserExceptionEof( "EOF" );
				}
	...
	public List<String> grabLines( String startToken, String endToken ) throws UrlParserException
	{
		...
		while (true)
		{
			...
			{
				log.info("found start token : " + startToken);
				if (line.contains(endToken))
				{
					log.info("found end token : " + endToken);
					int endPos = line.indexOf(endToken);
					int startPos = line.indexOf(startToken);
					if (endPos > startPos)
					{
						block.add(line.substring(startPos,endPos));
						block.add(endToken);
						currLine = null;
						return block;
					}
		...
		while (true)
		{
			...
			if (line.contains(endToken))
			{
				log.info("found end token - " + endToken + " block size: " + block.size());
				break;
			}
			line = readLine();
		}
